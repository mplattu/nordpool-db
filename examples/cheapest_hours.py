...
# Fetch yesterday
nordpool_db.update_data(prices_spot.hourly(areas=[AREA], end_date=dt_yesterday))
...
while (time_pointer < dt_yesterday_end):
    ...
    price = nordpool_db.get_price_value(AREA, time_pointer)
    rank = nordpool_db.get_price_rank(AREA, dt_yesterday_start, dt_yesterday_end, time_pointer)

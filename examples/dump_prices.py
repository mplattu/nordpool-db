...
# Fetch today
npdb.update_data(prices_spot.hourly(areas=AREAS, end_date=dt_today))

# Fetch yesterday
npdb.update_data(prices_spot.hourly(areas=AREAS, end_date=dt_yesterday))

# Fetch tomorrow
npdb.update_data(prices_spot.hourly(areas=AREAS, end_date=dt_tomorrow))
...
while (time_current <= time_end):
    ...
    for this_area in AREAS:
        price = npdb.get_price_value(this_area, time_current)
